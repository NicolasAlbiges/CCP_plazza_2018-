...
void Plazza::graph(int fd, char *buff)
{
    char op[] = " \n\t\r";
    std::pair<int, int> graph = {fd, atoi(word_array(buff, op)[1].c_str())};

    _ipc->pushGrapInfo(graph);
}
...
{
    command_t const arr_cmd[3] = {{"STATUS", &Plazza::status}, {"END", &Plazza::end}, {"GRAPH", &Plazza::graph}};
    char op[] = " \n\t\r";
    std::vector<std::string> arr = word_array(buffer, op);
...
std::string Plazza::createPizzaCmd(Pizza_t pizza)
{
...
int Plazza::algorithmBrowseKitchen(Pizza_t pizza)
{
    ...
    if (sendFd != -1) {
        *_ipc << sendFd;
        *_ipc << createPizzaCmd(pizza).c_str();
        return 1;
    }
    return 0;
}
...
void Plazza::run(Error *err)
{
    ...
    PlazzaIPC *tmp = _ipc;
    int cooks = _cooks;
    std::thread t = std::thread([tmp, cooks](){runGraphic(tmp, cooks);});
